...
class BaseJsonSchema:
    ...
    def generate_next_version_string(self, update_type: UpdateType = UpdateType.MinorUpdate) -> str:
        """
...
class DataProductSchema(BaseJsonSchema):
    ...
    MINOR_UPDATE_FIELDS = {"tableDescription"}
    ...
    def create_new_version(self) -> DataProductSchema:
        """
        ...
        self.version = self.generate_next_version_string(state)
        self.latest_version_key = DataProductConfig(self.data_product_name).schema_path(self.version).key
    ...
    def _detect_update_type(self) -> UpdateType:
        ...
        column_state = self._detect_column_differences()
        if column_state == UpdateType.Unchanged:
            changed_fields = self.changed_fields()
            if not changed_fields:
                return UpdateType.Unchanged
            if not changed_fields.difference(self.MINOR_UPDATE_FIELDS):
                return UpdateType.MinorUpdate
        else:
            return column_state

    def _detect_column_differences(self) -> UpdateType:
        ...
        for old_col in old_col_dict:
            if new_col_dict.get(old_col) is not None:
                if not old_col_dict[old_col]["type"] == new_col_dict[old_col]["type"]:
                    types_changed.add(old_col)
                if not old_col_dict[old_col]["description"] == new_col_dict[old_col]["description"]:
                    descriptions_changed.add(old_col)

        self.change_notification["removed_columns"] = list(removed_columns) if removed_columns else None
        self.change_notification["added_columns"] = list(added_columns) if added_columns else None
        self.change_notification["types_changed"] = list(types_changed) if types_changed else None
        self.change_notification["descriptions_changed"] = list(descriptions_changed) if descriptions_changed else None

        if any([removed_columns, types_changed]):
            return UpdateType.MajorUpdate
        elif any([added_columns, descriptions_changed]):
            return UpdateType.MinorUpdate
        elif not any([added_columns, types_changed, removed_columns, descriptions_changed]):
            return UpdateType.Unchanged
...
class DataProductMetadata(BaseJsonSchema):
    ...
    def create_new_version(self) -> DataProductMetadata:
        ...
        self.latest_version_key = DataProductConfig(self.data_product_name).metadata_path(self.version).key
        self.write_json_to_s3(self.latest_version_key)
